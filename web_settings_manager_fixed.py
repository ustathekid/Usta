...
try:
    ...
    # Quiet import - only show in debug mode
...
class WebSettingsManager(WebBaseManager):
    ...
    # Class-level flag to prevent multiple scheduler initializations
    ...
    def __init__(self):
        ...
        # Günlük otomatik indexleme için scheduler başlat (sadece bir kez)
        if not WebSettingsManager._scheduler_started:
            self._start_daily_scheduler()
            WebSettingsManager._scheduler_started = True
    ...
    def _restart_scheduler_silently(self):
        ...
        # Schedule primary indexing (silently)
        primary_time = auto_settings.get('primary_time', '07:55')
        schedule.every().day.at(primary_time).do(self._daily_auto_index, schedule_type='primary')
        
        # Schedule secondary indexing if enabled (silently)
        if auto_settings.get('secondary_enabled', False):
            secondary_time = auto_settings.get('secondary_time', '14:00')
            schedule.every().day.at(secondary_time).do(self._daily_auto_index, schedule_type='secondary')
    ...
    def update_auto_indexing_settings(self, settings):
        ...
        try:
            ...
            if self.save_config(config):
                # Restart scheduler with new settings (silently, without new log messages)
                self._restart_scheduler_silently()
                return True
            return False
